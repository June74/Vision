...
function updateOperation(snapshot: CalendarSetupSnapshot): void { const active = readActiveCreation(); if (!active) return; if (snapshot.status === "connected") { window.sessionStorage.removeItem(OPERATION_STORAGE_KEY); return; } const outcome = snapshot.status === "creating" ? "creating" : snapshot.actionRequired ? "action_required" : snapshot.status === "failed" && snapshot.retryable === false ? "definite_failure" : snapshot.status === "failed" ? "retryable" : active.outcome; window.sessionStorage.setItem(OPERATION_STORAGE_KEY, JSON.stringify({ ...active, outcome })); }
