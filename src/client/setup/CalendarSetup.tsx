...
export function CalendarSetup({ snapshot, session, onSnapshotChange }: { readonly snapshot: CalendarSetupSnapshot; readonly session: BrowserSession; readonly onSnapshotChange: (snapshot: CalendarSetupSnapshot) => void }): JSX.Element {
  ...
  async function run(action: NonNullable<typeof pending>, command: () => Promise<CalendarSetupSnapshot>): Promise<void> { setPending(action); setProblem(false); try { const next = await command(); setConfirmation(""); onSnapshotChange(next); updateOperation(next); } catch { setProblem(true); } finally { setPending(undefined); } }
  ...
  /** Re-discovers before clearing a terminal replay record, preserving retryability on network failure. */
  async function restart(): Promise<void> { setPending("discovering"); setProblem(false); try { const next = await discoverCalendars(session, snapshot.setupVersion); window.sessionStorage.removeItem(OPERATION_STORAGE_KEY); setConfirmation(""); onSnapshotChange(next); } catch { setProblem(true); } finally { setPending(undefined); } }
  ...
  return <section ref={panelRef} role="region" tabIndex={-1} className="setup-card" aria-label="Calendar setup state"><p className="setup-card__eyebrow">Calendar connection</p><h2 id="setup-title">{title}</h2><StatusBanner message={announcement(snapshot, pending, problem)} tone={problem || snapshot.actionRequired ? "warning" : "neutral"} />{renderControls(snapshot, confirmation, setConfirmation, pending, problem, operation, discover, restart, select, createOrReplay, refresh)}</section>;
}

/** Chooses controls and safe copy for the current authoritative setup outcome. */
function renderControls(snapshot: CalendarSetupSnapshot, confirmation: string, setConfirmation: (value: string) => void, pending: string | undefined, problem: boolean, operation: ActiveCreation | undefined, discover: () => Promise<void>, restart: () => Promise<void>, select: (id: string) => Promise<void>, create: () => Promise<void>, refresh: () => Promise<void>): JSX.Element {
  ...
  if (snapshot.status === "failed" && operation?.outcome === "definite_failure") return <><p>The calendar request was not completed. Start again by checking your owned Vision calendars.</p><button className="button button--primary" type="button" onClick={() => { window.sessionStorage.removeItem(OPERATION_STORAGE_KEY); void discover(); }}>Start setup again</button></>;
  if (snapshot.actionRequired && operation?.outcome === "definite_failure") return <><p>The calendar request was not completed. Start again by checking your owned Vision calendars.</p><button className="button button--primary" type="button" onClick={() => void restart()}>Start setup again</button></>;
...
function updateOperation(snapshot: CalendarSetupSnapshot): void { const active = readActiveCreation(); if (!active) return; if (snapshot.status === "connected") { window.sessionStorage.removeItem(OPERATION_STORAGE_KEY); return; } const outcome = snapshot.status === "creating" ? "creating" : snapshot.status === "failed" && snapshot.actionRequired && snapshot.retryable === undefined ? "definite_failure" : snapshot.actionRequired ? "action_required" : snapshot.status === "failed" ? "retryable" : active.outcome; window.sessionStorage.setItem(OPERATION_STORAGE_KEY, JSON.stringify({ ...active, outcome })); }
