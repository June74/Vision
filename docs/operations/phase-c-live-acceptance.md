...
| Preview deployment admission | The workflow is manually dispatched and has no repository migration step. The current local branch is clean at the reviewed Phase C tip, but the GitHub CLI credential is invalid; no push or workflow dispatch has occurred. | Pending GitHub re-authentication |
