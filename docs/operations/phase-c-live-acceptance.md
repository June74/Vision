...
| Preview deployment admission | The workflow was dispatched for the reviewed Phase C branch, but the `Admit one preview operation` selection job reportedly ended with exit code 1. The exact failed step and safe log message are not yet captured; no production deployment is authorized. | Open failure investigation |
