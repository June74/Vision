...
| Preview deployment admission | The workflow was dispatched for the reviewed Phase C branch, but `Admit one preview operation` rejected the context before deployment. The canonical context passes locally when intact; the likely cause is Windows PowerShell 5.1 stripping JSON quotes from the native `gh --raw-field` argument. The corrected `gh workflow run --json` standard-input dispatch is pending. | Contained; corrected dispatch pending |
...
| Database role and connection | A read-only preflight through the stored URL reached `neondb` with both `current_user` and `session_user` equal to `vision_app`; the host was direct/non-pooled. `vision_app` has `USAGE` on `public` but no `CREATE` privilege. Owner-side SQL Editor verification returned `neondb_owner` with `public CREATE = 1`. The attached ACL shows direct `vision_app` DML grants on all six Phase C tables, consistent with the established Phase B privilege manifest. | Pass |
| Phase C migrations | The target had 31 visible public tables before Phase C: the 29 table set expected through reviewed migration `0009`, plus the restore attestation table and an extra `playing_with_neon` table. No Drizzle migration journal was present. The schema-version-9 attestation closed the baseline gate, and the user reported successful application of reviewed migrations `0010`–`0013`. | Applied; runtime proof pending |
