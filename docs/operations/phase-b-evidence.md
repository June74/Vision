...
Gate 0 local verification is complete. The immutable candidate is not yet
frozen until a fresh whole-branch exact-tip review reports zero Critical and
zero Important findings and the reviewed tip is pushed through the permanent
privacy-safe Git adapter with closed Boolean equality. Task 8 live acceptance
remains blocked until those steps succeed.

The first frozen tip failed review because its permanent adapter pushed
symbolic `HEAD` instead of the canonical reviewed commit object. The repaired
