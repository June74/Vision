...
It recursively reads bounded regular files from the built client and five required generated evidence surfaces. It rejects plain, URI-encoded, fully percent-encoded, base64, and base64url forms of the protected sentinel. Client assets are separately checked for a closed list of server-only binding names.
...
Resolves a project root, verifies the fresh build-bound manifest, validates required evidence files, bounds each input to 16 MiB, rejects symbolic links, scans protected encodings and client secret-binding names, evaluates the Google call allowlist and Worker route manifest, sorts safe violations deterministically, and returns them without exposing matched content.
