...
`SafeLogEventSchema` is a strict Zod object containing only `requestId`, `action`, `outcome`, optional `errorCategory`, optional `diagnosticStage`, `durationMs`, `provider`, `retryCount`, `entityId`, and `entityIds`. `entityId` and every `entityIds` member must be UUIDs, preventing names and descriptions from entering the sink. `diagnosticStage` is a `z.enum` over the closed `AUTH_DIAGNOSTIC_STAGES` set, so it can only carry a literal authored in `src/server/auth/diagnostics.ts`. The log field is not environment-gated; only its response header/page rendering is preview-only. Existing prototype, hidden-field, and symbol rejection is unchanged.
