...
These Drizzle definitions declare every supported reviewed PostgreSQL key, owner-scoped foreign key, and named check: a common node envelope, typed events, governed edges, privacy-safe audit facts, explicit synchronization/recovery state, and the additive Phase C approval/execution boundary.
...
The Phase C tables keep encrypted proposal content separate from the durable
provider ledger. `proposal_domain` selects the existing per-owner/per-domain
key partition; it is controlled metadata, not event content.
