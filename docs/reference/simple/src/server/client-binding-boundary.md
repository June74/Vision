...
Synchronization suppression is included in the server-only temporary selector
values that built client assets must never contain.
...
The temporary preview-restore fields, acceptance selector, AI evidence minute,
and AI attestation belong to the runtime client-forbidden list. Their names may appear in the
server-only Worker bundle, but neither their names nor their values may enter a
browser asset. The complete eleven-selector value vocabulary is also forbidden
from browser assets.
