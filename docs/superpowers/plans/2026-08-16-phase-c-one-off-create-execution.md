...
- [x] **Step 1: Build a confirmed proposal fixture and fake ports.** Use the
...
- [x] **Step 2: Test stale target rejection.** A different current calendar
...
- [x] **Step 3: Test verified create.** Matching version claims once, calls
...
- [x] **Step 4: Test verified replay.** A seeded verified ledger record is
  returned without reading the target or calling provider create.
- [x] **Step 5: Test uncertain reconciliation.** An uncertain create followed
...
- [x] **Step 6: Test mismatch and undo.** A read-back field mismatch returns
...
- [x] **Step 7: Run the RED check.** Run
...
- [x] **Step 1: Add the port contracts.** Export these exact core types, with
...
- [x] **Step 2: Add exact preview matching and safe-audit helpers.** Match
...
- [x] **Step 3: Implement `executeConfirmedCalendarCreate`.** Enforce this
...
- [x] **Step 4: Implement `undoVerifiedCalendarCreate`.** Require a verified
  ...
  delete again.
- [x] **Step 5: Run the focused tests GREEN.** Use the Task 1 command and
...
- [x] **Step 1: Test calendar version reads.** Require the fixed Google origin
...
- [x] **Step 2: Test insert safety.** Require `POST
...
- [x] **Step 3: Test marker lookup, read-back, and delete.** Require
  `privateExtendedProperty` marker filtering, strict one-off event parsing,
  `DELETE` with `If-Match`, and `sendUpdates=none`.
- [x] **Step 4: Test outcome classification.** GET rejection is definite;
  ...
  the error boundary.
- [x] **Step 5: Run the adapter RED check.** Run
...
- [x] **Step 1: Add fixed-origin request and bounded JSON helpers.** Reuse the
...
- [x] **Step 2: Implement five methods.** Implement
  ...
  shape.
- [x] **Step 3: Run adapter tests GREEN.** Run the Task 3 command and require
...
- [x] **Step 1: Document the executor and adapter in plain language.** Define
...
- [x] **Step 2: Record the exact boundary.** Mark the execution core and
