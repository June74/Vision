...
export const phaseBSchemaManifest = {
  ...
  calendar_sync_maintenance: {
    columns: [
      ...
      ["credential_failure_checkpoint_version", "integer", false],
      ["credential_failure_category", "text", false],
      ["credential_failure_recorded_at", "timestamptz", false],
      ["created_at", "timestamptz", true],
      ["updated_at", "timestamptz", true],
    ],
    ...
    checks: [
      ...
      ["calendar_sync_maintenance_credential_category_valid", "credential_failure_category is null or credential_failure_category in ('authorization', 'concurrency', 'database', 'provider', 'payload_too_large', 'quota', 'schema', 'sync_token_invalid', 'transient')"],
      ["calendar_sync_maintenance_credential_marker_consistent", "(credential_failure_checkpoint_version is null) = (credential_failure_category is null) and (credential_failure_checkpoint_version is null) = (credential_failure_recorded_at is null)"],
      ["calendar_sync_maintenance_credential_version_non_negative", "credential_failure_checkpoint_version is null or credential_failure_checkpoint_version >= 0"],
  ...
  sync_channels: {
    ...
    checks: [
      ["sync_channels_activation_consistent", "(lifecycle = 'active' and activated_at is not null and retired_at is null) or (lifecycle = 'retired' and activated_at is not null and retired_at is not null) or lifecycle in ('pending', 'failed')"],
